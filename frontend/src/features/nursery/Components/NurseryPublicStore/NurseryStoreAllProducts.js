...
const NurseryStoreAllProducts = ({ nurseryPublicStore }) => {
    ...
    useEffect(() => {
        ...
        query.nursery = nurseryPublicStore._id; // Filter to this specific nursery

        dispatch(getAllProductsAsync(query));
    }, [location.search, dispatch, nurseryPublicStore._id]);
    ...
    const updateUrlParams = (key, value) => {
        ...
        // Ensure activeTab stays in the URL
        newParams.set('activeTab', 'products');
        
        navigate(`/nursery/store/view/${nurseryPublicStore._id}?${newParams.toString()}`);
    };
    ...
    return (
        <div className="container product-container mb-4 mb-md-5">
            ...
            <div className="p-2 d-flex flex-wrap justify-content-center align-item-center">
                <button onClick={() => handelSearchProductsByCategory("all")} className={`btn ${categoryList.includes('all') || categoryList.length === 0 ? 'btn-primary' : 'btn-secondary'} m-1`}>All</button>
                {categories && categories.map(cat => (
                    <button key={cat._id} onClick={() => handelSearchProductsByCategory(cat._id)} className={`btn ${categoryList.includes(cat._id) ? 'btn-primary' : 'btn-secondary'} m-1`}>
                        {cat.name}
                    </button>
                ))}
            </div>
